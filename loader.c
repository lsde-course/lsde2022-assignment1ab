...
// hash map needs long keys (large person ids), but unsigned int is enough for
// person offsets
...
void knows_line_handler(unsigned char nfields, char **tokens) {
  ...
  knows_person_offset =
      kh_value(person_offsets, kh_get(pht, person_offsets, knows_id));
...
int main(int argc, char *argv[]) {
  ...
  // first pass person, parse person, write to binary and store bin offset in
  // hash table
  ...
  person_map =
      (Person *)mmap(0, person_offset * sizeof(Person), PROT_READ | PROT_WRITE,
