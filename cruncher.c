...
void query(unsigned short qid, unsigned short artist, unsigned short areltd[],
           unsigned short bdstart, unsigned short bdend) {
  unsigned int person_offset, friend_offset;
  unsigned long knows_offset, knows_offset2;

  Person *person, *knows;
  signed char score,
      *score_cache = (signed char *)malloc(person_length / sizeof(Person));
  ...
  for (person_offset = 0; person_offset < person_length / sizeof(Person);
       person_offset++) {
  ...
  for (person_offset = 0; person_offset < person_length / sizeof(Person);
    ...
    for (knows_offset = person->knows_first;
         knows_offset < person->knows_first + person->knows_n; knows_offset++) {
      ...
      if (score_cache[friend_offset] != -1)
        continue;
  ...
  for (result_idx = 0; result_idx < result_length; result_idx++) {
    ...
    for (knows_offset2 = results[result_idx].knows_first;
         knows_offset2 <
         results[result_idx].knows_first + results[result_idx].knows_n;
         knows_offset2++) {
...
int main(int argc, char *argv[]) {
  ...
  person_map =
      (Person *)mmapr(makepath(argv[1], "person", "bin"), &person_length);
  interest_map = (unsigned short *)mmapr(makepath(argv[1], "interest", "bin"),
                                         &interest_length);
  knows_map =
      (unsigned int *)mmapr(makepath(argv[1], "knows", "bin"), &knows_length);
